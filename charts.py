...
def plot_line_fidelity_portfolio(summaries: list):
    ...
    curr_balances = {} # Balance(s): {'ROTH IRA': 0, 'Individual': 0, '401K': 0} or just {'checking': 0}
...
def plot_line_savings_by_month(savings_transactions: List[BankTransaction], fidelity_transactions: List[FidelityTransaction], annual_income: float):
    """
    Plots monthly savings rate as a line chart.
    ...
    - annual_income: Gross annual income, used as the savings rate denominator.
    """
    ...
    while savings_ptr >= 0 or fidelity_ptr >= 0:
        ...
        if savings_ptr < 0:
            ...
            if curr_period is not None and curr_period.month != datetime.strptime(fidelity_transactions[fidelity_ptr]['date'], "%m/%d/%Y").month:
                savings_rates.append((sum(curr_balances.values()) - sum(prev_balances.values())) / annual_income)
